...
from network_architecture.myVGG import vgg11_bn
from network_architecture.generic_VNet import VNet_class
from data_augmentation.data_augmentation import default_3D_augmentation_params, default_2D_augmentation_params, get_default_augmentation, get_patch_size
from dataloading.dataset_load import load_dataset, DataLoader3D, unpack_dataset
from loss_function.Loss_functions import JI_and_Focal_loss, AutomaticWeightedLoss, softmax_helper, sum_tensor
...
class Trainer(nn.Module):
    ...
    def initialize(self, training=True):
        ...
        self.all_val_eval_metrics_class =[]
    ...
    def finish_online_evaluation(self):
        ...
        self.all_val_eval_metrics_class.append(np.mean(global_acc_per_class))
        self.all_val_eval_metrics_seg.append(np.mean(global_dc_per_class))
    ...
    def update_eval_criterion_MA(self):
        ...
        if self.val_eval_criterion_MA_class is None:
            if len(self.all_val_eval_metrics) == 0:
                ...
                self.val_eval_criterion_MA_class = self.all_val_eval_metrics[-1]#liuyiyao -2
        else:
            if len(self.all_val_eval_metrics) == 0:
                """
                ...
                self.val_eval_criterion_MA_class = self.val_eval_criterion_alpha * self.val_eval_criterion_MA_class + (
                        1 - self.val_eval_criterion_alpha) * \
                                                   self.all_val_eval_metrics[-2]#liuyiyao -2
    ...
    def plot_progress(self):
        ...
        try:
            ...
            if len(self.all_val_eval_metrics_class) == len(x_values):
                ax2.plot(x_values, self.all_val_eval_metrics_class, color='g', ls='--', label="acc")
            if len(self.all_val_eval_metrics_seg) == len(x_values):
                ax2.plot(x_values, self.all_val_eval_metrics_seg, color='black', ls='--', label="dc")
            ax.set_xlabel("epoch")
            ax.set_ylabel("loss")
