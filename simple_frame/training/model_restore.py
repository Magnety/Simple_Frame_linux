...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    """
    This is a utility function to load any simple_frame trainer from a pkl. It will recursively search
    simple_frame.trainig.network_training for the file that contains the trainer and instantiate it with the arguments saved in the pkl file. If checkpoint
    ...
    init = info['init']
    name = info['name']
    search_in =  simple_frame.__path__[0]+"/"+ "training"+"/"+ "network_training"
    tr = recursive_find_python_class([search_in], name, current_module="simple_frame.training.network_training")
    ...
    if tr is None:
        raise RuntimeError("Could not find the model trainer specified in checkpoint in simple_frame.trainig.network_training. If it "
                           "is not located there, please move it or change the code of restore_model. Your model "
                           "trainer can be located in any directory within simple_frame.trainig.network_training (search is recursive)."
    ...
    """if len(init) == 7:
        print("warning: this model seems to have been saved with a previous version of simple_frame. Attempting to load it "
